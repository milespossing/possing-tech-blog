...
2. We have a few different kinds of error handling:
   ...
   2. A wrapping error: C# has these rather nicely, the [`System.Exception` class](https://learn.microsoft.com/en-us/dotnet/api/system.exception?view=net-8.0) actually includes a ctor for this very purpose. The benefit here is that we maintain our **call stack** (an integral part of any exception system) but we're able to add local information to it (ie readable reasons for what went wrong, making ops easier; or basic information on the parameters provided to the function making debugging and reproduction easier).
